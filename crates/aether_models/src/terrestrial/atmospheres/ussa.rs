...
impl USSA {
    ...
    const base_geometric_heights: [f64; 13] = [
        0.0, 11019.0, 20063.0, 32162.0, 47350.0, 51413.0, 71802.0, 86000.0, 91000.0, 110000.0,
        120000.0, 500000.0, 1000000.0,
    ];
    const base_geometric_temperatures: [f64; 13] = [
        288.15, 216.65, 216.65, 228.650, 270.65, 270.65, 214.650, 186.87, 186.87, 240.0, 360.0,
        999.24, 1000.0,
    ];
    const base_geometric_pressures: [f64; 13] = [
        101.325e3, 226.32e2, 547.48e1, 868.01, 110.90, 669.38e-1, 395.64e-2, 373.38e-3, 153.81e-3,
        710.42e-5, 253.82e-5, 302.36e-6, 751.38e-8,
    ];
    const base_geometric_densities: [f64; 13] = [
        1.225, 0.35822, 8.8035e-2, 1.3225e-2, 1.4275e-3, 8.6160e-4, 6.4211e-5, 6.958e-6, 2.860e-6,
        9.708e-8, 2.222e-8, 5.215e-13, 3.561e-15,
    ];
    const base_lapse_rates: [f64; 12] = [
        -6.5e-3, 0.0, 1.0e-3, 2.8e-3, 0.0, -2.8e-3, -2.0e-3, 0.0, 0.0, 12.0, 12.0, 0.0,
    ];
    ...
    pub fn temperature(&self, geometric_height: f64) -> Result<f64, Constraint> {
        let base_index = self
            .get_base_index(geometric_height)
            .expect("Could not determine base index.");
        let gradient_index_result = self.get_gradient_index(base_index);
        match gradient_index_result {
            Ok(gradient_index) => {
                if geometric_height < 86000.0 {
                    return Ok(Self::base_geometric_temperatures[base_index]
                        + Self::base_lapse_rates[gradient_index]
                            * (geometric_height - Self::base_geometric_heights[base_index]));
                    ...
                    let temperature = t_c
                        + A * ((1.0
                            - ((geometric_height - Self::base_geometric_heights[base_index]) / a)
                                .powf(2.0))
                        .exp());
                    return Ok(temperature);
                    ...
                    let t_inf = 1000.0;
                    let lambda = 0.01875;
                    let eta = (geometric_height - Self::base_geometric_heights[9])
                        * (Self::r_0 + Self::base_geometric_heights[9])
                        / (Self::r_0 + geometric_height)
                        * 1e-3;
            ...
            Ok(gradient_index) => {
                ...
                if lapse_rate != 0.0 {
                    let pressure = base_pressure
                        * ((base_temperature / temperature).powf(
                            (Self::g_0 * Self::air_molar_mass) / (Self::R_Star * lapse_rate),
                        ));
                    return Ok(pressure);
                } else {
                    let pressure = base_pressure
                        * ((-Self::g_0
                            * Self::air_molar_mass
                            * (geometric_height - Self::base_geometric_heights[base_index]))
                            / (Self::R_Star * temperature))
                            .exp();
                    return Ok(pressure);
                }
            }
            Err(Constraint::NoGradient) => return Err("No Gradient"),
        }
    }
    ...
    pub fn dynamic_viscosity(&self, geometric_height: f64) -> Result<f64, &'static str> {
        let temperature = self
            .temperature(geometric_height)
            .expect("Could not determine temperature.");
        let viscosity = Self::dynamic_viscosity_nist_A * (temperature).powf(1.5)
