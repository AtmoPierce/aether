...
impl ISA {
    ...
    pub fn new() -> Self {
        ISA {
            base_geopotential_altitudes: [
                -5000.0, 0.0, 11000.0, 20000.0, 32000.0, 47000.0, 51000.0, 71000.0, 80000.0,
            ],
            base_geopotential_densities: [
                1.93047, 1.225, 3.63918e-1, 8.80345e-2, 1.32249e-2, 1.42752e-3, 8.616e-4,
                6.42105e-5, 1.57004e-5,
            ],
            base_temperatures: [
                320.65, 288.15, 216.65, 216.65, 228.65, 270.65, 270.65, 214.650, 196.65,
            ],
    ...
    pub fn gravity(geometric_altitude: f64) -> f64 {
        Self::g_0
            * (Self::earth_radius_nominal / (Self::earth_radius_nominal + geometric_altitude))
                .powf(2.0)
    }
    pub fn geopotential_altitude(&self, geometric_altitude: f64) -> f64 {
        (Self::earth_radius_nominal * geometric_altitude)
            / (Self::earth_radius_nominal + geometric_altitude)
    }
    pub fn geometric_altitude(&self, geopotential_altitude: f64) -> f64 {
        (Self::earth_radius_nominal * geopotential_altitude)
            / (Self::earth_radius_nominal - geopotential_altitude)
    }
    ...
    pub fn temperature(&self, geopotential_altitude: f64) -> Result<f64, &'static str> {
        ...
        match gradient_index_result {
            Ok(gradient_index) => {
                return Ok(self.base_temperatures[base_index]
                    + self.gradients[gradient_index]
                        * (geopotential_altitude - self.base_geopotential_altitudes[base_index]));
            }
        ...
        match gradient_index_result {
            Ok(gradient_index) => {
                ...
                if gradient != 0.0 {
                    ...
                    let t2 = 1.0
                        + (gradient / base_temperature)
                            * (geopotential_altitude
                                - self.base_geopotential_altitudes[base_index]);
                    let t3 = -Self::g_0 / (Self::R * gradient);
                    let pressure = t1 * t2.powf(t3);
                    return Ok(pressure);
                } else {
                    let pressure = base_pressure
                        * ((-Self::g_0 / (Self::R * temperature))
                            * (geopotential_altitude
                                - self.base_geopotential_altitudes[base_index]))
                            .exp();
                    return Ok(pressure);
