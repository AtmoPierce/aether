mod tests{
    ...
    fn test_euler_dcm_roundtrip(){
        ...
        let dcm1: DirectionCosineMatrix<f64, Body<f64>, Body<f64>> = DirectionCosineMatrix::from(euler);
        let euler_back = Euler::from(&dcm1);
        let dcm2: DirectionCosineMatrix<f64, Body<f64>, Body<f64>> = DirectionCosineMatrix::from(euler_back);
